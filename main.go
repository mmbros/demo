...
func (s *selectParent) apply(e Element, p *pather) {
	...
	if parent := e.ElementParent(); parent != nil {
...
func (n *Node) ElementParent() Element {
	return Element(n.parent)
}

func (n *Node) ElementChildren() []Element {
	elements := make([]Element, len(n.children))
...
func (n *Node) MatchTag(tag string) bool {
	return n.name == tag
}
...
func main() {
	...
	path, err := CompilePath("DIV//SPAN")
	if err != nil {
		fmt.Println(err)
	}
	res := path.FindElements(root)
	for j, e := range res {
		fmt.Printf("%d) %v\n", j, e)
	}
